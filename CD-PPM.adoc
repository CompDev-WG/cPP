= collaborative PP-Module for Companion Devices
:showtitle:
:toc: macro
:toclevels: 7
:sectnums:
...
== Acknowledgements
This collaborative Protection Profile Module (PP-Module) was developed by the {iTC-longname} Working Groups (WG) also known as {iTC-shortname} with representatives from industry, Government agencies, Common Criteria Test Laboratories, and members of academia.
